import { addChartUsingPost } from '@/services/backend/chartController';
import { getMyPointsUsingGet } from '@/services/backend/pointsController';
...
const Welcome: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      // 需要取到上传的原始数据file→file→originFileObj(原始数据)
      const res = await addChartUsingPost(params, {}, values.file.file.originFileObj);
      // 正常情况下，如果没有返回值就分析失败，有，就分析成功
      if (!res?.data) {
        message.error('分析失败');
      } else {
        ...
        if (!chartOption) {
          ...
          // 从后端得到响应结果之后，把响应结果设置到图表状态里1653726240902787074_0.6138419838097393
          setChart(res.data);
    ...
    <div className="add-chart">
      ...
      <Row gutter={24}>
        ...
        <Col span={12}>
          <Card
            title="智能分析"
            extra={<span style={{ color: 'red' }}>使用次数剩余：{point}</span>}
          >
            <Form
              // 表单名称改为addChart
              name="addChart"
